...
export const generateViralScriptStream = async (
  ...
  const prompt = `
    ...
    📏 **분량 요구사항 (절대 준수!)**
    - **원본과 거의 동일한 길이**로 작성 (요약 금지)
    - 타임스탬프를 원본만큼 많이 사용 (예: 00:00, 00:30, 01:00, 01:30, 02:00, 03:00...)
    - 각 타임스탬프마다 **최소 4-6문장**의 대사 작성
    - 디테일한 설명, 구체적 사례, 데이터를 풍부하게 포함
    - 짧게 요약하지 말고 **풀버전으로 길게** 작성
