...
const getClient = () => {
  ...
  if (!apiKey) {
    throw new Error("Claude API 키가 설정되지 않았습니다. API 키를 먼저 입력해주세요.");
  }
  return new Anthropic({ apiKey, dangerouslyAllowBrowser: true });
};

// 1. Analyze structure and suggest topics
export const analyzeTranscript = async (
  originalTranscript: string
): Promise<AnalysisResult> => {
  const client = getClient();

  const prompt = `당신은 100만 구독자를 보유한 유튜브 전략가입니다.
...
JSON 형식으로만 응답하세요:
{
  "structureSummary": "타이밍과 전개 패턴만 포함한 구조적 분석",
  "suggestedTopics": ["완전히 다른 분야의 주제 1", "주제 2", "주제 3"]
}`;
...
// 2. Generate script with streaming
export const generateViralScriptStream = async (
  ...
  const prompt = `당신은 전문 유튜브 스크립트 작가입니다.
